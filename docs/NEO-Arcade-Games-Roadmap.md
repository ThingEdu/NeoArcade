# NEO Arcade — Kế hoạch Bộ Game Vận Động cho Trẻ
...
NEO Arcade là **cánh tay "vận động"** trong vũ trụ Dế Foundation — bổ sung cho
...
### 3.2 Bắt Dế (Camera) — tương tác tay qua webcam  ➡️ ĐÃ TÁCH sang [NeoAiSport](https://github.com/ThingEdu/NeoAiSport)

> Các game **thị giác AI** (camera + tay/cử chỉ/mặt/tư thế) có stack riêng nặng (opencv + mediapipe)
> nên tách khỏi NEO Arcade thành nền tảng riêng **NeoAiSport**, để NEO Arcade nhẹ, hợp NEO cấu hình thấp.
> Mô tả gốc giữ lại để tham khảo:
...
đúng cách DeBlue đang dạy qua mô phỏng. NEO Arcade (vận động) + DeBlue (tư duy) = 2 cửa vào
...
| **3** ➡️ | **Game thị giác → tách sang NeoAiSport** (Bắt Dế đã chuyển; repo/domain/stack riêng) | webcam + MediaPipe | giữ NEO Arcade nhẹ cho NEO yếu |
| **4** | Nhảy Xa · Thăng Bằng · Nhịp · Nước Rút | đế cảm ứng, nghiêng | phủ hết nhóm vận động |
| **5** | Đóng gói catalog **NEO Arcade launcher** + đưa lên [[project_neoplay_status]] | — | phát hành |
