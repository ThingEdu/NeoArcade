...
class BasketRenderer:
    ...
    def draw(self, ctrl, ev, lb, dt):
        ...
        if ctrl.state == ctrl.MENU:
            self._menu(ctrl, lb)
            wordmark(self.screen, self.f_sm, self.logo_sm, "Bóng Rổ Dế · NEO Arcade")
            return
        s = self.screen
        s.blit(self.sky, (0, 0))
        ...
        if ctrl.state == ctrl.RESULT:
            self._result(ctrl)
        wordmark(s, self.f_sm, self.logo_sm, "Bóng Rổ Dế · NEO Arcade")
    ...
    def _menu(self, ctrl, lb):
        ...
        center_text(s, self.f_sm, "NEO Arcade · canh lực ném vào rổ", C.W // 2, 200, C.GREEN_CRICKET)
