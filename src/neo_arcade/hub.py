"""NEO Arcade — màn hình tổng (launcher): chọn 1 trong các game rồi mở.
...
from neo_arcade.ui.render import make_sky
from neo_arcade.ui.sprites import draw_cricket, font, load_logo, load_maker_viet, scale_to
from neo_arcade.ui.widgets import center_text, draw_text, round_rect

GAMES = [
    dict(title="FlappyDe", move="Phản xạ · 1 nút", control="Nút bấm / nhảy",
         module="neo_arcade.app", args=["--profile", "keyboard"],
         accent=C.BLUE_ELECTRIC, icon="cricket"),
    dict(title="Đua Xe Dế", move="Lái né · tinh", control="Cần lái / nút",
         module="neo_arcade.dexe.app", args=["--profile", "keyboard"],
         accent=C.ORANGE_HOT, icon="car"),
    dict(title="Bóng Rổ Dế", move="Ném / nhắm", control="Nút canh lực",
         module="neo_arcade.bongro.app", args=["--profile", "keyboard"],
         accent=C.GREEN_CRICKET, icon="basket"),
    dict(title="Đấm Bốc", move="Sức mạnh", control="Đập nút",
         module="neo_arcade.damboc.app", args=["--profile", "keyboard"],
...
class Hub:
    def __init__(self):
        ...
        pygame.display.set_caption("NEO Arcade — Dế Foundation")
    ...
    def draw(self):
        ...
        center_text(s, self.f_hero, "NEO Arcade", C.W // 2, 132, C.BLUE_ELECTRIC)
    ...
    def _footer(self):
        ...
        draw_text(s, self.f_sm, "NEO Arcade", x, yc - 9, C.INK, shadow=False)
        if self.logo_mv:
